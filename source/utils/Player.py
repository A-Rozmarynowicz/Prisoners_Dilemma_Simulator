...
class Bot_Player(Player):
    def __init__(self, ID : int, action_space : Type[Action], strategy : Strategy):
        super().__init__(ID=ID, action_space=action_space)
        self.strategy = strategy

    def Make_Move(self, action_history : list[tuple[Action, ...]]) -> Action:
        return self.strategy.Make_Move(ID=self.ID, action_space=self.action_space, action_history=action_history)